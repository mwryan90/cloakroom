...
`setup` also registers cloakroom in **Power BI Desktop's External Tools ribbon** (alongside tools like Tabular Editor), so the admin UI is one click away while you're looking at your model — the button starts the UI or reuses a running instance. Writing the registration needs an elevated (Administrator) terminal; setup degrades gracefully without it, and `--no-external-tool` skips it entirely.

Opens a localhost-only page that connects to your model: search and browse tables/columns (likely-sensitive ones are flagged), see distinct counts and real sample values, and save masking rules straight into `masking.yaml`. Order doesn't matter — if you open your Power BI file after the UI, it connects automatically within ~15 seconds.

Masking more than one MCP server? The UI manages them all: a header picker switches between sources, with rules and schema discovery scoped per source while **tokens stay shared** — the same entity keeps the same token in every connection. Sources wrapped in generic mode (`--adapter none`) show an honest coverage summary instead of a column browser: what the global sweep masks there, and what isn't protected until a value is learned via a source with discovery.

Generic sources can also be warmed up manually: enumerate sensitive values yourself on a trusted channel (e.g. `SELECT DISTINCT` in your database portal — never via the agent, whose first read is exactly the exposure warm-up prevents) and paste them into the coverage card's **Seed values** form. They register into the shared store and are masked everywhere immediately. A **mask all text** button on each table covers every untagged text column in one click; columns you mark **Not sensitive** get a *skipped* badge and can be restored at any time. Every sampled value shows a live **"agent sees"** preview — its token, "auto-token on first query", or "(excluded)". Assign meaningful-but-safe tokens by hand ("BigRetailer"), leave blank for automatic numbering, or tick **skip** to exclude placeholders like "UNKNOWN" from masking entirely. High-cardinality columns show the first 50 values; the rest are tokenized automatically. Wrongly flagged columns get a **Not sensitive** button that dismisses the suggestion for good. A **token decoder** in the header searches both directions — type "Company 5" to see who it really is, or a real name to find its token — so you can cross-reference anything the agent tells you. The page shows real data by design: it is for the human data owner and binds to 127.0.0.1 only.
